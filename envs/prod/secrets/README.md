...
Values that Terraform must never see. Encrypted with SOPS against the `acme-prod-secrets`
KMS key, which exists only for this environment: a leaked prod credential cannot decrypt
anything belonging to staging.
...
sops envs/prod/secrets/app-secrets.enc.yaml

# Or start from the plaintext template, then encrypt and delete the plaintext.
sops -e envs/prod/secrets/app-secrets.yaml > envs/prod/secrets/app-secrets.enc.yaml
rm envs/prod/secrets/app-secrets.yaml

# Push values into SSM. Decrypts in memory; nothing plaintext is written to disk.
./scripts/secrets-push.sh prod --dry-run
./scripts/secrets-push.sh prod
```
