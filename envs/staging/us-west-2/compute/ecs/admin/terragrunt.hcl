...
inputs = {
  ...
  cpu            = local.env.admin.cpu
  memory         = local.env.admin.memory
  desired_count  = local.env.admin.desired_count

  attach_alb            = true
  alb_listener_arn      = dependency.alb.outputs.https_listener_arn
  alb_security_group_id = dependency.alb.outputs.security_group_id
  host_headers          = [local.region.admin_host]
  ...
  enable_autoscaling = local.env.admin.enable_autoscaling
  min_count          = local.env.admin.min_count
  max_count          = local.env.admin.max_count
