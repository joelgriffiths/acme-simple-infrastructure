...
dependency "alb" {
  # Internal only: reachable from inside the VPC, never from the internet.
...
inputs = {
  ...
  cpu            = local.env.api.cpu
  memory         = local.env.api.memory
  desired_count  = local.env.api.desired_count

  attach_alb            = true
  alb_listener_arn      = dependency.alb.outputs.https_listener_arn
  alb_security_group_id = dependency.alb.outputs.security_group_id
  host_headers          = [local.region.api_host]
  ...
  environment = {
    ...
    PROVIDER_MODE           = local.env.provider_mode
    PUBLIC_BOOKING_BASE_URL = "https://${local.region.patient_host}"
  }
  ...
  enable_autoscaling = local.env.api.enable_autoscaling
  min_count          = local.env.api.min_count
  max_count          = local.env.api.max_count
