...
inputs = {
  ...
  cpu            = local.env.clinician.cpu
  memory         = local.env.clinician.memory
  desired_count  = local.env.clinician.desired_count

  attach_alb            = true
  alb_listener_arn      = dependency.alb.outputs.https_listener_arn
  alb_security_group_id = dependency.alb.outputs.security_group_id
  host_headers          = [local.region.clinician_host]
  ...
  enable_autoscaling = local.env.clinician.enable_autoscaling
  min_count          = local.env.clinician.min_count
  max_count          = local.env.clinician.max_count
