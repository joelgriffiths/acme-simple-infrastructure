...
dependency "sqs_reminders" {
  ...
  mock_outputs = {
    queue_url = "https://sqs.${local.mock_region}.amazonaws.com/${local.mock_account}/mock-reminders"
    queue_arn = "arn:aws:sqs:${local.mock_region}:${local.mock_account}:mock-reminders"
  }
  mock_outputs_allowed_terraform_commands = ["validate", "plan"]
}
dependency "sqs_rebooking" {
  ...
  mock_outputs = {
    queue_url = "https://sqs.${local.mock_region}.amazonaws.com/${local.mock_account}/mock-rebooking"
    queue_arn = "arn:aws:sqs:${local.mock_region}:${local.mock_account}:mock-rebooking"
  }
  mock_outputs_allowed_terraform_commands = ["validate", "plan"]
}
dependency "cluster" {
...
inputs = {
  ...
  cpu           = local.env.scheduler.cpu
  memory        = local.env.scheduler.memory
  desired_count = local.env.scheduler.desired_count
  ...
  environment = {
    ...
    DB_PORT             = "5432"
    QUEUE_URL_REMINDERS = dependency.sqs_reminders.outputs.queue_url
    QUEUE_URL_REBOOKING = dependency.sqs_rebooking.outputs.queue_url
  }
  ...
  task_policy_json = jsonencode({
    ...
    Statement = [{
      ...
      Resource = [dependency.sqs_reminders.outputs.queue_arn, dependency.sqs_rebooking.outputs.queue_arn]
    }]
  })
