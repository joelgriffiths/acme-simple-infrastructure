# Region-scoped values for staging/us-west-2: availability zones, address space, DNS.
# Sizing and environment identity live one level up in envs/staging/env.hcl.
locals {
  region = "us-west-2"
  ...
  #
  # Staging uses its own subdomains rather than reusing the prod apex in a second VPC.
  ...
  public_domain  = "staging.dumbidea.us"
  private_domain = "staging.dumbidea.internal"

  public_zone_id = "Z00000000000000000STG" # placeholder public Route53 hosted zone
